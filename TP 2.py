...
resultados_df = pd.DataFrame({
    ...
    'Cantidad de Atributos': [len(eval(grupo)) for grupo in grupos]  # Calcular cantidad de atributos en cada grupo
})

# Ordenar por cantidad de atributos y mantener el orden en el gráfico
resultados_ordenados = resultados_df.sort_values('Cantidad de Atributos')

# Graficar exactitudes ordenadas
plt.figure(figsize=(10, 6))
x = range(len(resultados_ordenados))  
plt.plot(x, resultados_ordenados['Exactitud'], marker='o', linestyle='-', color='black', label='Exactitud')
plt.xticks(x, resultados_ordenados['Grupo'], rotation=45, ha='right')
plt.xlabel('Grupos de atributos')
plt.ylabel('Exactitud')
plt.title('Exactitud en función de los grupos de atributos seleccionados')
plt.grid(axisss='y', linestyle='--', alpha=0.7)
plt.legend()
plt.tight_layout()
plt.show()
...
for idx, crit in enumerate(criterios):
    cm = matrices_confusion_por_criterio_y_altura[(10, crit)]
    
    # Graficar la matriz de confusión
    plt.subplot(1, len(criterios), idx + 1)
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", cbar=False, xticklabels=range(10), yticklabels=range(10))
    plt.title(f"Criterio: {crit} (Profundidad: 10)")
    plt.xlabel("Predicción")
    plt.ylabel("Real")
